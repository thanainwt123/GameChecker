...
// ========================================
// CPU / GPU ที่มีประสิทธิภาพใกล้เคียงกัน
// ========================================

function getSimilarCPUs(value) {

    const score = cpuScore[value];

    const options = Array.from(
        document.querySelectorAll("#cpu option")
    );

    return options
        .filter(option =>
            option.value !== value &&
            cpuScore[option.value] === score
        )
        .map(option => {

            const name = option.textContent.trim();

            const brandClass = option.value.startsWith("r")
                ? "brand-amd"
                : "brand-intel";

            return `<span class="${brandClass}">${name}</span>`;
...
function getSimilarGPUs(value) {

    const score = gpuScore[value];

    const options = Array.from(
        document.querySelectorAll("#gpu option")
    );

    return options
        .filter(option =>
            option.value !== value &&
            gpuScore[option.value] === score
        )
        .map(option => {

            const name = option.textContent.trim();

            const brandClass = option.value.startsWith("rx")
                ? "brand-amd"
                : "brand-nvidia";
...
function getCPUName(score) {
    ...
    const cpuNames = {
        ...
        3:
            '<span class="brand-intel">Intel Core i3-12100F / i3-13100F / i5-10400</span> / <span class="brand-amd">Ryzen 5 3600</span>',
        ...
        6:
            '<span class="brand-intel">Intel Core i5-12600K / i5-13400F / i5-13500 / i7-12700</span> / <span class="brand-amd">Ryzen 5 7500F / Ryzen 7 5700X</span>',

        7:
            '<span class="brand-intel">Intel Core i5-13600K / i5-14400F / i5-14500 / i9-12900K</span> / <span class="brand-amd">Ryzen 5 7600 / Ryzen 5 7600X / Ryzen 7 5800X3D</span>',

        8:
            '<span class="brand-intel">Intel Core i5-14600K / i7-13700 / i7-14700 / Core Ultra 5 245K</span> / <span class="brand-amd">Ryzen 7 7700 / Ryzen 7 7700X</span>',

        9:
            '<span class="brand-intel">Intel Core i7-14700K / i9-13900K / i9-14900K / Core Ultra 7 265K / Core Ultra 9 285K</span> / <span class="brand-amd">Ryzen 7 7800X3D / Ryzen 7 9700X / Ryzen 7 9800X3D / Ryzen 9 9900X / Ryzen 9 9950X / Ryzen 9 9950X3D</span>'
...
// ========================================
// ชื่อ CPU ที่ผู้ใช้เลือก
// ========================================

function getSelectedCPUName(value) {

    const option = document.querySelector(
        `#cpu option[value="${value}"]`
    );
...
// ========================================
// ชื่อ GPU ที่ผู้ใช้เลือก
// ========================================

function getSelectedGPUName(value) {

    const option = document.querySelector(
        `#gpu option[value="${value}"]`
    );
...
function getGPUName(score) {
    ...
    const gpuNames = {
        ...
        5:
            '<span class="brand-nvidia">RTX 3060 / RTX 5050</span> / <span class="brand-amd">RX 5700 XT</span>',

        6:
            '<span class="brand-nvidia">RTX 3070 / RTX 4060 / RTX 5060</span> / <span class="brand-amd">RX 6700 XT / RX 7600 / RX 9060</span>',

        7:
            '<span class="brand-nvidia">RTX 3080 / RTX 4060 Ti / RTX 5060 Ti</span> / <span class="brand-amd">RX 6800 / RX 7700 XT / RX 9060 XT</span>',

        8:
            '<span class="brand-nvidia">RTX 3090 / RTX 4070</span> / <span class="brand-amd">RX 6900 XT / RX 7800 XT</span>',

        9:
            '<span class="brand-nvidia">RTX 4070 Super / RTX 4070 Ti / RTX 5070</span> / <span class="brand-amd">RX 7900 XT / RX 9070</span>',

        10:
            '<span class="brand-nvidia">RTX 4070 Ti Super / RTX 5070 Ti</span> / <span class="brand-amd">RX 7900 XTX / RX 9070 XT</span>',

        11:
            '<span class="brand-nvidia">RTX 4080</span>',

        12:
            '<span class="brand-nvidia">RTX 4090 / RTX 5080</span>',

        13:
            '<span class="brand-nvidia">RTX 5090</span>'
...
function checkGame() {
    ...
    const similarCPUs = getSimilarCPUs(userCPU);
    const similarGPUs = getSimilarGPUs(userGPU);
    ...
    document.getElementById(
        ...
        <div class="your-spec-title">
            สเปคของคุณ
        </div>

        <p>
            <strong>CPU ของคุณ:</strong>
            ${getSelectedCPUName(userCPU)}
        </p>

        <p>
            <strong>GPU ของคุณ:</strong>
            ${getSelectedGPUName(userGPU)}
        </p>
        ...
        <!-- ==============================
             รุ่นที่มีประสิทธิภาพใกล้เคียงกัน
             ============================== -->

        <div class="similar-specs">

            <div class="similar-title">
                CPU ที่มีประสิทธิภาพใกล้เคียงกัน (ของคุณ)
        </div>
 
        <div class="similar-text">
            ${similarCPUs || "ไม่มีข้อมูลรุ่นที่ใกล้เคียงกัน"}
        </div>

        <div class="similar-title gpu-title">
            GPU ที่มีประสิทธิภาพใกล้เคียงกัน (ของคุณ)
        </div>

        <div class="similar-text">
            ${similarGPUs || "ไม่มีข้อมูลรุ่นที่ใกล้เคียงกัน"}
        </div>
